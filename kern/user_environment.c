...
void env_free(struct Env *e)
{
	...
	for (int indx = 0; indx < e->page_WS_max_size; ++indx)
	{
		unmap_frame(e->env_page_directory, (void *)env_page_ws_get_virtual_address(e, indx));
		env_page_ws_clear_entry(e, indx);
	}

	kfree(e->ptr_pageWorkingSet);
	for(int pos = 0; pos < PDX(USER_TOP); pos++)
	{
		if((e->env_page_directory[pos] & 0xFFFFF000) != 0)
		{
			uint32 add = (e->env_page_directory[pos] & 0xFFFFF000);
			struct Frame_Info* f = to_frame_info(add);
			if (f->references != 0)
			{
				f->references = 0;
				free_frame(f);
			}
			e->env_page_directory[pos] = 0;
		}
	}
