...
void * create_page_table(uint32 *ptr_page_directory, const uint32 virtual_address)
{
	...
	uint32 vir_add=(uint32) kmalloc(PAGE_SIZE),tmp_phys_add=kheap_physical_address(vir_add),indx=PDX(virtual_address);
	ptr_page_directory[indx]= CONSTRUCT_ENTRY(tmp_phys_add,PERM_USER|PERM_PRESENT|PERM_WRITEABLE);
	//memset((void *)vir_add ,0,PAGE_SIZE);
	//tlb_invalidate(ptr_page_directory,(void*)vir_add);
	tlbflush();
	return (void*)vir_add;
}
...
void freeMem(struct Env* e, uint32 virtual_address, uint32 size)
{
	...
	//TODO: [PROJECT 2022 - [12] User Heap] freeMem() [Kernel Side]
		...
		uint32 vir_indx = virtual_address, *PT = NULL,inx,vir_WS,vir_indx2,ind3;
		while ( vir_indx <= size )
		{
			pf_remove_env_page(e, vir_indx);
			vir_indx += PAGE_SIZE;
		}
		//2. Free ONLY pages that are resident in the working set from the memory
		for (inx = 0; inx < e->page_WS_max_size; inx++)
		{
			vir_WS = ROUNDDOWN(e->ptr_pageWorkingSet[inx].virtual_address, PAGE_SIZE);
			if (vir_WS <= size && vir_WS >= virtual_address)
			{
				unmap_frame(e->env_page_directory, (void*)vir_WS);
				env_page_ws_clear_entry(e, inx);
			}
		}
		for (vir_indx2 = virtual_address; vir_indx2 <= size; vir_indx2 += PAGE_SIZE)
		{
			get_page_table(e->env_page_directory, (void*)vir_indx2, &PT);
			bool P_Emp = 1;
			if (PT != NULL)
			{
				for (ind3 = 0; ind3 < 1024; ind3++)
				{
					if (PT[ind3] & PERM_PRESENT)
					{
						P_Emp = 0; break;
					}
				}
				if (P_Emp == 1)
				{
					pd_clear_page_dir_entry(e,vir_indx2);
					kfree((void*)PT);
				}
			}
			tlbflush();
