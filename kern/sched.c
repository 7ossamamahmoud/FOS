...
void sched_init_MLFQ(uint8 numOfLevels, uint8 *quantumOfEachLevel)
{
	...
	int s = sizeof(struct Env_Queue)*numOfLevels,qu = sizeof(uint8)*numOfLevels,indx=0,indx2;
	env_ready_queues = kmalloc(s);
	while (indx < numOfLevels)
	{
		init_queue(&env_ready_queues[indx]);
		indx++;
	}
	quantums = kmalloc(qu);
	for (indx2 = 0;indx2< numOfLevels;indx2++)
		quantums[indx2] = quantumOfEachLevel[indx2];
...
struct Env* fos_scheduler_MLFQ()
{
	...
	int indx = 0;
	if (curenv != NULL)
		enqueue(&env_ready_queues[envque], curenv);

		while(indx < num_of_ready_queues)
		{
			if (queue_size(&env_ready_queues[indx]) > 0)
			{
				e = dequeue(&env_ready_queues[indx]);
				kclock_set_quantum(quantums[indx]);
				envque = (indx < num_of_ready_queues - 1 ) ? indx + 1 : num_of_ready_queues - 1;
				return e;
			}
			indx++;
		}
