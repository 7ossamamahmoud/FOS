...
void page_fault_handler(struct Env * curenv, uint32 fault_va)
{
	...
	while(indx < curenv->page_WS_max_size)
	{
		if(curenv->ptr_pageWorkingSet[curenv->page_last_WS_index].empty == 1)
		{
			pos = curenv->page_last_WS_index++;
	...
	if(pos != -1)
	{
		...
		map_frame(curenv->env_page_directory, F_Info, (void *) fault_va,PERM_WRITEABLE | PERM_USER );
		curenv->ptr_pageWorkingSet[pos].virtual_address = fault_va;
		curenv->ptr_pageWorkingSet[pos].empty = 0;
	...
	else
	{
		...
		int ind = 0;
		pos = -1;
		while(ind < curenv->page_WS_max_size)
		{
			int ida =PTX(curenv->ptr_pageWorkingSet[curenv->page_last_WS_index].virtual_address);
			get_page_table(curenv->env_page_directory, (void *)curenv->ptr_pageWorkingSet[curenv->page_last_WS_index].virtual_address, &ptr_PT);
			if((ptr_PT[ida] & PERM_MODIFIED) == 0 && (ptr_PT[ida] & PERM_USED) == 0)
			{
				pos = curenv->page_last_WS_index++;
				curenv->page_last_WS_index = curenv->page_last_WS_index % curenv->page_WS_max_size;
				break;
		...
		if(pos == -1)
		{
			int index = 0;
			uint32 *ptr_pt = NULL;
			while(index < curenv->page_WS_max_size)
			{
				uint32 ida =PTX(curenv->ptr_pageWorkingSet[curenv->page_last_WS_index].virtual_address);
				get_page_table(curenv->env_page_directory, (void *)curenv->ptr_pageWorkingSet[curenv->page_last_WS_index].virtual_address, &ptr_pt);
				if((ptr_pt[ida] & PERM_USED) == 0)
				{
					pos = curenv->page_last_WS_index++;
					curenv->page_last_WS_index = curenv->page_last_WS_index % curenv->page_WS_max_size;
					break;
				}
				ptr_pt[ida] &= (~PERM_USED);
				curenv->page_last_WS_index++;
				curenv->page_last_WS_index = curenv->page_last_WS_index % curenv->page_WS_max_size;
				index++;
			}
		...
		if(pos == -1)
		{
			...
			while (ind < curenv->page_WS_max_size)
			{
				uint32 ida2 =PTX(curenv->ptr_pageWorkingSet[curenv->page_last_WS_index].virtual_address);
				get_page_table(curenv->env_page_directory, (void *)curenv->ptr_pageWorkingSet[curenv->page_last_WS_index].virtual_address, &ptr_pt);
				if(((ptr_pt[ida2] & PERM_USED) == 0 && (ptr_pt[ida2] & PERM_MODIFIED)) == 0)
				{
					pos = curenv->page_last_WS_index++;
					curenv->page_last_WS_index = curenv->page_last_WS_index % curenv->page_WS_max_size;
					break;
		...
		uint32 *ptr_Ptt = NULL;
		struct Frame_Info *f = get_frame_info(curenv->env_page_directory, (void *)curenv->ptr_pageWorkingSet[pos].virtual_address, &ptr_Ptt);
		int ida3=PTX(curenv->ptr_pageWorkingSet[pos].virtual_address);
		map_frame(curenv->env_page_directory, f, (void*)fault_va, PERM_PRESENT | PERM_USER | PERM_WRITEABLE);
		if((ptr_Ptt[ida3] & PERM_MODIFIED))
			pf_update_env_page(curenv, (void*)curenv->ptr_pageWorkingSet[pos].virtual_address, f);
		ptr_Ptt[ida3] = ptr_Ptt[ida3] & (~PERM_MODIFIED);
		unmap_frame(curenv->env_page_directory, (void*)curenv->ptr_pageWorkingSet[pos].virtual_address);
		if (pf_read_env_page(curenv,(void*)fault_va) == E_PAGE_NOT_EXIST_IN_PF)
			(fault_va < USTACKTOP && fault_va >= USTACKBOTTOM) ? pf_add_empty_env_page(curenv, fault_va, 0) : panic("Warning: Vicious Entry !!!!!\n");
		curenv->ptr_pageWorkingSet[pos].virtual_address = fault_va;
	}
